...
//       // Fetch a single blog with author info from user_profiles
...
//       // Prepare blog object for components
//       const blog = {
//         ...blogData,
//         readTime: `${Math.ceil(parseFloat(blogData.read_time))} min`,
//         date: new Date(blogData.created_at).toLocaleDateString(),
//         author: blogData.user_profiles?.name || "author",
//         authorRole: blogData.user_profiles?.role || "author",
//         authorImage: blogData.user_profiles?.profile_image || '/images/placeholder.png',
//         tags: ['Tech', 'SaaS', 'Innovation'], // Replace with real tag data later
//         authorBio: 'Tech enthusiast and SaaS builder.', // Optional static or dynamic bio
//         ...blogData.content, // Optional: if blog has JSON fields like introduction, etc.
//       };

//       setBlog(blog);

//       // Fetch recent blog previews for footer
//       const { data: recent, error: recentError } = await supabase
//         .from('blogs')
//         .select('id, title, blog_image, read_time')
...
//           <div className="h-8 w-32 bg-gray-300 animate-pulse rounded mb-4 mx-auto"></div>
//           <p className="text-gray-600">Loading blog post...</p>
//         </div>
//       </div>
//     );
...
const BlogDetail = () => {
  ...
  const fetchBlogData = useCallback(async () => {
    try {
      ...
      // Prepare blog object for components without hard-coded data
      const blog = {
        ...blogData,
        readTime: `${Math.ceil(parseFloat(blogData.read_time))} min`,
        date: new Date(blogData.created_at).toLocaleDateString(),
        author: blogData.user_profiles?.name,
        authorRole: blogData.user_profiles?.role,
        authorImage: blogData.user_profiles?.profile_image || '/images/placeholder.png',
      };
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-[#F8F7FF]">
        <div className="text-center p-8 max-w-md">
          ...
          <a
            href="/blogs"
            className="px-4 py-2 bg-[#6B4EFF] text-white rounded-lg hover:bg-[#5A3FE0] transition-colors"
          >
            Back to Blogs
          </a>
        </div>
